...
export const parseCodeBlocks = (text: string) => {
  ...
  text = text.replace(
    ...
    (match, openingFence, lang, code, closingFence) => {
      let highlighted = lang ? emphasize.highlight(lang, code) : emphasize.highlightAuto(code)
      if (!highlighted.value.trim()) {
        highlighted = emphasize.highlight("plaintext", code)
      }

      return `${chalk.dim(openingFence)}${highlighted.value}${chalk.dim(closingFence)}`
    }
  )
  ...
  //     const highlighted = emphasize.highlightAuto(code)
